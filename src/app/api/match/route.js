...
async function getLiveBets() {
    ...
    try {
        ...
        // If still no matches in DB, use hardcoded matches as fallback
        if (!matches || !matches?.data || matches?.data?.length < 30) {
            console.log("Database empty, using hardcoded matches");
            // Get hardcoded matches for current day (max 50 for match route)
            const apiMatches = getHardcodedMatches(50);
        ...
        if (matches?.data) {
            const parsedData = await JSON.parse(matches?.data || `[]`);
            return Array.isArray(parsedData) ? parsedData : [];
        } else {
            return [];
        }
    } catch (error) {
        console.error("Error in getLiveBets:", error);
        if (error?.code === 500 || error?.status === 500 || !error?.status) {
            ErrorReport(error);
        }
        // Fallback to hardcoded matches if database fails
        try {
            const apiMatches = getHardcodedMatches(50);
            return apiMatches || [];
        } catch (fallbackError) {
            console.error("Fallback to hardcoded matches failed:", fallbackError);
        }
        return [];
    }
}
...
async function getExtractedMatches(matches) {
    ...
    for (let match of matches) {
        ...
        // Check if the match starts between 5 PM and 11 PM
        if (matchHour >= 17 && matchHour < 23) {
...
export async function POST(request) {
    ...
    try {
        ...
        if (BetAmount < 500) {
            throw new CustomError(
                705,
                "minimum bet amount is 500 , kindly recharge or increase the bet amount."
            );
        }
